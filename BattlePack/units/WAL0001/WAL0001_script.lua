...
local BWeapons = import('/lua/BattlePackweapons.lua')
local SC2DisruptorWeapon02 = BWeapons.SC2DisruptorWeapon02
...
WAL0001 = Class(AWalkingLandUnit) {

    DeathThreadDestructionWaitTime = 2,

    Weapons = {
        DeathWeapon = Class(AIFCommanderDeathWeapon) {},
        RightDisruptor = Class(SC2DisruptorWeapon02) {},
		...
		AAGun1 = Class(AAASonicPulseBatteryWeapon) {},
    ...
    OnCreate = function(self)
		...
		self:HideBone('AA_Upgrade01', true)   
		self:HideBone('AA_Upgrade02', true)      
		self:HideBone('LeftUpgrade', true)	 
    ...
    WarpInEffectThread = function(self)
        ...
        WaitSeconds(2.1)
        self:SetMesh('/units/WAL0001/WAL0001_PhaseShield_mesh', true)
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh == 'HeatSink' then
            local wep = self:GetWeaponByLabel('RightDisruptor')
			local aa = self:GetWeaponByLabel('AAGun1')
			local aa2 = self:GetWeaponByLabel('AAGun2')
            wep:AddDamageMod(bp.ZephyrDamageMod)        
			aa:AddDamageMod(bp.AADamageMod)  
			aa2:AddDamageMod(bp.AADamageMod)  
        elseif enh == 'HeatSinkRemove' then
            local bp = self:GetBlueprint().Enhancements['HeatSink']
            if not bp then return end
            local wep = self:GetWeaponByLabel('RightDisruptor')
			local aa = self:GetWeaponByLabel('AAGun1')
			local aa2 = self:GetWeaponByLabel('AAGun2')
