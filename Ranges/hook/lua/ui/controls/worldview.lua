...
function createReclaimRangeRingPreviews(units)
    units = units or {}
	if ACSdata.previews.reclaimrange then
        for _, u in EntityCategoryFilterDown(categories.ENGINEER, units) do
            local bp = u:GetBlueprint()
            local radius = bp.Economy.MaxBuildDistance
            if radius then
                if (not ACSdata.rings.reclaimrange[radius]) then
					local texture = acs_modpath..'textures/reclaim_ring.dds'
					createRing(ACSdata.rings.reclaimrange, texture, 28, 0, 0, 0, 0)
                end
            end
        end
		for _, u in EntityCategoryFilterDown(categories.FACTORY, units) do
                if (not ACSdata.rings.reclaimrange[28]) then
					local texture = acs_modpath..'textures/reclaim_ring.dds'
					createRing(ACSdata.rings.reclaimrange, texture, 28, 0, 0, 0, 0)
                end
        end
    end
    ACSdata.isReclaimPreviewAlive = true
end
...
function createRing(group, texture, radius, x1, x2, y1, y2)
    if not group[radius] then
        ...
        ring:SetScale({math.floor(2.05*(radius + x1) + x2), 0, math.floor(2.05*(radius + y1)) + y2})
...
function createReclaimPreviewOfCurrentSelection()
    createReclaimRangeRingPreviews(GetSelectedUnits() or {})
end
...
function removePreview()
    ...
    for n, group in ACSdata.rings do
		if group ~= ACSdata.rings.reclaimrange then
...
function updateReclaimPreview()
    if (not ACSdata.isReclaimPreviewAlive) then
        createReclaimPreviewOfCurrentSelection()
    end
	for __, ring in ACSdata.rings.reclaimrange do
		ring:SetPosition(GetMouseWorldPos())
	end
end

function removeReclaimPreview()
    if (not ACSdata.isReclaimPreviewAlive) then
        return
    end
    for n, group in ACSdata.rings do
    ...
    ACSdata.isReclaimPreviewAlive = false
end
...
WorldView = Class(oldWorldView, Control) {
    ...
    OnUpdateCursor = function(self)
        if (self.isPreviewBuildrange or self.isPreviewAttackrange or self.isPreviewReclaimrange) then
            if isAcceptablePreviewMode(CM.GetCommandMode()) then
				...
				if IsKeyDown(18) then
					updateReclaimPreview()
				else
					removeReclaimPreview()
				end
            end
        end
	...
	SetPreviewReclaimrange = function(self, bool)
        self.isPreviewReclaimrange = bool
        ACSdata.previews.reclaimrange = bool
        removeReclaimPreview()
    end,
