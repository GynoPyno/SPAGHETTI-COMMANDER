...
function DisperseMove()
    ...
    while index < unitcount do -- Need to be able to change iterator manually once loop is on last unit the first time, to reset it
        ...
        FixOrders(unit)
        if ordercounts[index] == -1 then
            ordercounts[index] = ordercountg
        end
        ...
        local beginAction,endAction,action,counter,actionAlwaysMixed = nil,nil,nil,ordercounts[index],false
        local alwaysMix = {"AggressiveMove", "Guard", "Move", "Patrol", "Teleport"}

        local action2 = unitOrders[ordercounts[index]].CommandType
        while unitOrders[counter].CommandType == action2 do
            ...
            if priorOrders[index] == -1 and unitOrders[beginAction - 1] ~= nil then 
                priorOrders[index] = unitOrders[beginAction - 1]
            end
            ...
            if created_distribution_table == 1 then -- Only take orders from order distribution table once they are there (after first loop thru units where last unit determines them)
                orderDistribution[index] = orderDistribution[index] + beginAction
                    ...
                    for i = beginAction, endAction do -- For all orders find closest unit to them that doesnt have a first order yet, running it like this forces even distribution
                        ...
                        for i2 = 1, unitcount do -- Run thru all the units looking for closest unit to current order that isnt already taken (has a first order already)
                            if orderDistribution[i2] == -1 then -- Dont bother with units that are already taken, waste of cpu calculating distance
                                ...
                                if priorOrders[i2] ~= -1 then -- If this unit has a different order queued prior to attack orders, use that order's position to determine closest queued attack order instead
                                    position = priorOrders[i2].Position
                                end
                        ...
                        if cunit ~= 1234567 then
                            orderDistribution[cunit] = i - beginAction -- Save index of closest order to unit as variable in order distribution table at that unit's index in it
                        end
                    end
                end
