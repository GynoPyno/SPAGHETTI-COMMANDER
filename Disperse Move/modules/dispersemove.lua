...
function DisperseMove()
	...
	unit = curSelection[index]
		...
		ordercount2 = table.getn(unitOrders)
		...
		action2 = unitOrders[ordercount2].CommandType
        while unitOrders[counter].CommandType == action2 do
			...
			if index == 1 and ranonce == 0 then
				actiondif = endAction - beginAction
			end
			if endAction - beginAction > actiondif then
				actiondif = endAction - beginAction
			end
			ahead = actiondif - (endAction - beginAction)
			
			lostorders = 0
			
			if created_distribution_table == 1 then
			...
			if created_distribution_table == 0 and ahead == 0 and ranonce == 1 then
				orderDistribution = {}
			...
			if index == table.getn(curSelection) and ranonce == 0 then 
				ranonce = 1
				index = 0
			end
