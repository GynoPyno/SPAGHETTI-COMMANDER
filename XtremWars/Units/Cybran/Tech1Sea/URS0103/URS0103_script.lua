...
URS0103 = Class(CSeaUnit) {
    ...
    OnStopBeingBuilt = function(self,builder,layer)
		...
		self.Trash:Add(CreateRotator(self, 'Cybran_Radar01', 'y', nil, 90, 0, 0))
		self.Trash:Add(CreateRotator(self, 'Front_Radar01', 'y', nil, 90, 0, 0))
		self.Trash:Add(CreateRotator(self, 'Front_Radar02', 'y', nil, -90, 0, 0))
            ...
            if( self.Walking ) then
                if( old == 'Stopped' ) then
                    if( self.SwitchAnims ) then
                        ...
                        self.AnimManip:PlayAnim(self:GetBlueprint().Display.AnimationWalk, true):SetRate(self:GetBlueprint().Display.AnimationWalkRate or 1.5)
                    else
                        self.AnimManip:SetRate(1.5)
                    end
    ...
    TransformThread = function(self, land)
        ...
        if( land ) then
            ...
            --self:SetCollisionShape( 'Box', bp.CollisionOffsetX or 0,(bp.CollisionOffsetY + (bp.SizeY*2.0)) or 0,bp.CollisionOffsetZ or 0, bp.SizeX * scale, bp.SizeY * scale, bp.SizeZ * scale )
