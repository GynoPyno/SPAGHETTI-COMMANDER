...
DUE0401 = Class(TMobileFactoryUnit) {
    Weapons = {
	...
	DeathThread = function(self)
		# Spawn an units (temp energy being)
        local position = self:GetPosition()
		local spiritUnit = CreateUnitHPR('URL0301_RAMBO', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)	
		local spiritUnit = CreateUnitHPR('UEL0301_RAMBO', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)	
		local spiritUnit = CreateUnitHPR('URL0301_RAMBO', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)	
		local spiritUnit = CreateUnitHPR('XSL0301_RAMBO', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)		
		local spiritUnit = CreateUnitHPR('SPIRIT0403', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)	
		local spiritUnit = CreateUnitHPR('SPIRIT0403', self:GetArmy(), position[1], position[2], position[3], 0, 0, 0)	
