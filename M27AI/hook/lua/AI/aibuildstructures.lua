...
function AddToBuildQueue(aiBrain, builder, whatToBuild, buildLocation, relative)
    ...
    if aiBrain.M27AI == false then
        ...
        if bDebugMessages == true then
            LOG('About to send build structure command')
...
function M27BuildStructureAtLocation(oBuilder, sBuildingType, tBuildLocation)
    ...
    local iBuilderFaction = M27Utilities.GetFactionFromBP(oBuilderBP)
    local sBuildingBlueprintID = M27Utilities.GetBlueprintIDFromBuildingTypeAndFaction(sBuildingType, iBuilderFaction)
...
function AIExecuteBuildStructure(aiBrain, builder, buildingType, closeToBuilder, relative, buildingTemplate, baseTemplate, reference, NearMarkerType)
    ...
    if not aiBrain.M27AI then
        ...
        if buildingType == 'T1LandFactory' or buildingType == 'T1EnergyProduction' then
            ...
            local iDistFromStart = M27Utilities.GetDistanceBetweenPositions(builderPos, M27MapInfo.PlayerStartPoints[iArmy])
            ...
            if buildingType == 'T1LandFactory' then
                ...
                if iDistFromStart <= iBuildDistance then
                    ...
                    --M27Utilities.DrawLocations(M27MapInfo.tResourceNearStart[iArmy][1], nil, 1, 50, false)
                    tBuildAdjacentTo = M27MapInfo.tResourceNearStart[iArmy][1]
            ...
            elseif buildingType == 'T1EnergyProduction' then
                tBuildAdjacentTo = M27Utilities.GetAllUnitPositions(M27Utilities.GetAllCategoryUnitsNearPosition(aiBrain, categories.STRUCTURE * categories.FACTORY - categories.NAVAL, builderPos, iBuildDistance + 4 + 2, false))
            ...
            if tBuildAdjacentTo == nil or tBuildAdjacentTo[1] == nil then
                ...
                relativeLoc = M27Utilities.GetAdjacencyLocationForTarget(tBuildAdjacentTo, sBuildingTypeToBuildBy, buildingType, buildingTemplate, true, aiBrain, true, builderPos, iBuildDistance, false, true)
                --function GetAdjacencyLocationForTarget(tablePosTarget, TargetBuildingType, NewBuildingType, buildingTemplate, bCheckValid, aiBrain, bReturnOnlyBestMatch, pBuilderPos, iBuilderMaxDistance, bIgnoreOutsideBuildArea, bBetterIfNoReclaim)function GetAdjacencyLocationForTarget(tablePosTarget, TargetBuildingType, NewBuildingType, bCheckValid, aiBrain, bReturnOnlyBestMatch, pBuilderPos, iBuilderMaxDistance, bIgnoreOutsideBuildArea, bBetterIfNoReclaim)
