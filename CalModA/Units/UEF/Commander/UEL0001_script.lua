...
UEL0001 = ClassUnit(ACUUnit) {
    Weapons = {
        ...
        Laser = ClassWeapon(TDFHiroPlasmaCannon) {},
    ...
    OnStopBeingBuilt = function(self, builder, layer)
        ...
        self:SetWeaponEnabledByLabel('AA', false)
    ...
    CreateEnhancement = function(self, enh)
        ...
        elseif enh == 'LightShield' then
            self:AddToggleCap('RULEUTC_ShieldToggle')
            self:CreateShield(bp)
            self:SetEnergyMaintenanceConsumptionOverride(bp.MaintenanceConsumptionPerSecondEnergy or 0)
            self:SetMaintenanceConsumptionActive()
        elseif enh == 'LightShieldRemove' then
            self:DestroyShield()
        ...
        elseif enh =='AA' then
            self:SetWeaponEnabledByLabel('AA', true)
        elseif enh =='AARemove' then
            self:SetWeaponEnabledByLabel('AA', false)
        ...
        elseif enh =='LaserCannonRemove' then
            self:SetWeaponEnabledByLabel('Laser', false)
             local bp = self:GetBlueprint().Enhancements['LaserCannon']
            if not bp then return end
            local wep = self:GetWeaponByLabel('RightZephyr')
            wep:AddDamageMod(-bp.ZephyrDamageMod)
            local bpDisrupt = self:GetBlueprint().Weapon[1].MaxRadius
