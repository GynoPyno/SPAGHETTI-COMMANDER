...
local MissileWeapon = import('/lua/terranweapons.lua').TSAMLauncher 
...
uel0402 = Class(TWalkingLandUnit) {
    ...
    Weapons = {
        plasma_repeater = Class(TDFHeavyPlasmaCannonWeapon) {}, 
        Missile_Pod = Class(MissileWeapon) {
            OnCreate = function(self) 
                MissileWeapon.OnCreate(self)
                -- Initialization of the missile counter 
                self.MissileCounter = 1
                -- A value of "true" indicates an odd numbered missile being fired
                self.OddEven = true  
            end, 
            
            CreateProjectileAtMuzzle = function(self, muzzle)
                -- Hyjacking CreateProjectileAtMuzzle
                local proj = MissileWeapon.CreateProjectileAtMuzzle(self, muzzle)
                if not proj or proj:BeenDestroyed()then
                    return proj
                end                
                --* Custom CreateProjectileAtMuzzle script *--                
                if not self.unit:IsDead() and self.unit:GetWeaponByLabel('Missile_Pod'):GetCurrentTarget() then                                  
                    -- Get the current target of this weapon
                    local myTarget = self.unit:GetWeaponByLabel('Missile_Pod'):GetCurrentTarget()                
                    --- Get the x, y, z, location of the target
                    local pos = myTarget:GetPosition()                                                                                               
                    -- Determines the angle from center
                    local angle = (2*math.pi) / self.unit.NumMissiles                   
                    -- Stagger the projectile spread between Odd and Even projectiles
                    -- Odd spread farther out and Even move closer in
                    local spreadMulti = nil
                    if self.OddEven == true then
                        self.OddEven = false
                        spreadMulti = self.unit.MissleSpread * 2.5
                    else
                        self.OddEven = true
                        spreadMulti = self.unit.MissleSpread / 4.5                    
                    end                                        
                    -- Do the math
                    local xVec = pos[1] + math.sin(self.MissileCounter*angle) * spreadMulti
                    local zVec = pos[3] + math.cos(self.MissileCounter*angle) * spreadMulti                  
                    local vPos = xVec,zVec
                    local gPos = GetSurfaceHeight( xVec, zVec )
                    local modpos = {xVec, gPos, zVec}                    
                    -- Pass the location information to the projectile 
                    proj:SetNewTargetGround(modpos)                                                                       
                    if self.MissileCounter >= self.unit.NumMissiles  then
                        -- Resets the counter
                        self.MissileCounter = 1
                        self.OddEven = true  
                    else
                        -- Increment the counter
                        self.MissileCounter = self.MissileCounter + 1
                    end                        
                end                 
                --* End of custom CreateProjectileAtMuzzle script *--                     
                return proj
            end,            
                                              
            OnLostTarget = function(self)
                MissileWeapon.OnLostTarget(self)
                self.MissileCounter = 1
                self.OddEven = true  
            end          
        },   
       ...
       plasma_repeater = Class(TDFHeavyPlasmaCannonWeapon) {},
       emp_right = Class(TDFOverchargeWeapon) {
           CreateProjectileAtMuzzle = function(self, muzzle)
               local proj = TDFOverchargeWeapon.CreateProjectileAtMuzzle(self, muzzle)
       ...
       emp_left = Class(TDFOverchargeWeapon) {
           CreateProjectileAtMuzzle = function(self, muzzle)
               local proj = TDFOverchargeWeapon.CreateProjectileAtMuzzle(self, muzzle)
               local data = self:GetBlueprint().DamageToShields
               if proj and not proj:BeenDestroyed() then
    ...
    OnCreate = function(self,builder,layer)
        ...
        self.MissilePod = self:GetWeaponByLabel('Missile_Pod')
        -- Number of missiles in eash salvo 
        self.NumMissiles = self.MissilePod:GetBlueprint().ProjectilesPerOnFire 
        -- Adjusts the width of the missile dispersal
        self.MissleSpread = self.MissilePod:GetBlueprint().DamageRadius         
    ...
    StompEffects = function(self)
        ...
        for i = 1, 2 do
            ...
            self:ShakeCamera(1, 0.25, 0.125, 0.25)
